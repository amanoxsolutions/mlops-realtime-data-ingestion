...
def create(_, __):
    ...
    if application_status == "RUNNING":
        ...
        logger.info(
            "Cannot start the application in the current state: {application_status}. The application must be in the READY state to start it."
        )
        return
...
def get_application_status(application_name: str) -> str:
    flink_app = client.describe_application(
        ApplicationName=application_name, IncludeAdditionalDetails=False
    )
