...
def lambda_handler(event, context):
    try:
        ...
        if request == "delete":
            domain_apps = delete_sagemaker_studio_apps(SAGEMAKER_DOMAIN_ID)
...
def delete_sagemaker_studio_apps(domain_id: str) -> List[str]:
    ...
    for app in apps:
        ...
        if app_name != SAGEMAKER_APP_NAME and status != "Deleting" and status != "Deleted":
            ...
            domain_apps.append(app_name)
    return domain_apps


def wait_studio_app_deletion(domain_id: str, app_names: List[str]):
