...
def lambda_handler(event, context):
    ...
    try:
        ...
        next_token, sagemaker_domain_list = list_existing_sagemake_domains()
        while next_token:
            next_token, additional_domain_list = list_existing_sagemake_domains(next_token)
            sagemaker_domain_list.extend(additional_domain_list)         
        # we can only return data as simple key-value pairs
        # so we convert the list of domains to a comma separated string
        sagemaker_domains = ",".join(sagemaker_domain_list)
...
def list_existing_sagemake_domains(next_token: str = None) -> Tuple[str, List[str]]:
