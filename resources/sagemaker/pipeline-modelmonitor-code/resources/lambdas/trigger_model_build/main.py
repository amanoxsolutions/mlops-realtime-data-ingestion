...
def lambda_handler(_, __):
    ...
    pipeline_name = (
        f"sagemaker-{SAGEMAKER_PROJECT_NAME}-{SAGEMAKER_PROJECT_ID}-modelbuild"
    )
    try:
        response = cp_client.start_pipeline_execution(name=pipeline_name)
        print(
            f"Triggered the pipeline {pipeline_name} with execution id {response['pipelineExecutionId']}"
        )
