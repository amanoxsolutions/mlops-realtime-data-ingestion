...
LOCAL_TRANSFORM_DIR = f"{PROCESSING_FOLDER_PREFIX}/transform"
LOCAL_EVALUATION_DIR = f"{PROCESSING_FOLDER_PREFIX}/evaluation"
# Prefix, will be used to identify our project
S3_BUILD_PREFIX = "build"
BASE_PREFIX = "BlockchainForecast"
# Resources names
MODEL_PACKAGE_GROUP_NAME = f"{BASE_PREFIX}PackageGroup"
PIPELINE_NAME = f"{BASE_PREFIX}Pipeline"
PROCESSING_STEP_NAME = f"{BASE_PREFIX}PreprocessingData"
PROCESSING_JOB_NAME = f"{BASE_PREFIX}/PreprocessingJob"
TRAINING_JOB_NAME = f"{BASE_PREFIX}-DeepAR-training"
TRAINING_STEP_NAME = f"Train{BASE_PREFIX}Model"
CREATE_MODEL_STEP_NAME = f"{BASE_PREFIX}CreateModel"
MODEL_TRAINING_JOB_NAME = f"{BASE_PREFIX}/DeeparTrainingJob"
TRANSFORM_STEP_NAME = f"{BASE_PREFIX}Transform"
TRANSFORM_JOB_NAME = f"{BASE_PREFIX}/TransformOutputsProcessingJob"
PROCESSING_TRANSFORM_STEP_NAME = f"{BASE_PREFIX}ProcessingTransformOutputs"
MODEL_QUALITY_CHECK_STEP_NAME = f"{BASE_PREFIX}ModelQualityCheck"
EVALUATION_JOB_NAME = f"{BASE_PREFIX}/DeeparEvaluationJob"
EVALUATION_REPORT_NAME = f"{BASE_PREFIX}EvaluationReport"
EVALUATION_STEP_NAME = f"{BASE_PREFIX}EvaluateModel"
REGISTER_MODEL_STEP_NAME = f"{BASE_PREFIX}RegisterModel"
CONDITON_STEP_NAME = f"{BASE_PREFIX}CheckMSECondition"
FAIL_STEP_NAME = f"{BASE_PREFIX}Fail"
# Instances
...
def get_pipeline(
    ...
    #
    model_path = f"s3://{default_bucket}/{S3_BUILD_PREFIX}/model_training/output"
    ...
    deepar_estimator = Estimator(
        ...
        base_job_name=MODEL_TRAINING_JOB_NAME,
    ...
    transform_output_path = f"s3://{default_bucket}/{S3_BUILD_PREFIX}/batch_transform/output"
    ...
    # following format:
    # target, quantile0.1, quantile0.5, quantile0.9
    ...
    eval_step_args = evaluate_processor.run(
        outputs=[
            ProcessingOutput(output_name="transform", source=LOCAL_TRANSFORM_DIR),
            ProcessingOutput(output_name="evaluation", source=LOCAL_EVALUATION_DIR)
        ],
        code=os.path.join(BASE_DIR, "evaluate.py"),
        arguments=[
            "--transform-outputs", f"{transform_output_path}/test-inputs.json.out",
            "--target-col", target_col
        ],
    )
    ...
    model_quality_check_config = ModelQualityCheckConfig(
        baseline_dataset=Join(
            on="/",
            values=[
                step_eval.properties.ProcessingOutputConfig.Outputs["transform"].S3Output.S3Uri,
                "/targets-transforms.csv"
            ]),
        dataset_format=DatasetFormat.csv(header=True),
        output_s3_uri=Join(on='/', values=['s3:/', default_bucket, S3_BUILD_PREFIX, ExecutionVariables.PIPELINE_EXECUTION_ID, 'modelqualitycheckstep']),
        problem_type='Regression',
        inference_attribute='quantile0.5',
    ...
    preprocessing_step_args = model.register(
    ...
    cond_lte = ConditionLessThanOrEqualTo(
        left=JsonGet(
            ...
            json_path="deepar_metrics.rmse.value"
        ),
        right=6.0,
    )
